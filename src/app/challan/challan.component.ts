...
export class ChallanComponent implements OnInit {
  ...
  initialiseInvites() {
    this.Form = this.fb.group({
      ...
      ChallanAmount : [null,Validators.required],
  ...
  onSubmit(){
    ...
    if(this.Form.get('ChallanDate').value != null){
      var dateVall = this.Form.get('ChallanDate').value;
      var newDatee = dateVall.getDate() +'/' + (dateVall.getMonth()+1) + '/'+ dateVall.getFullYear();
      this.Form.get('ChallanDate').setValue(newDatee);
    }
